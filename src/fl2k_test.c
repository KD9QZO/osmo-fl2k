...
/*! \brief Structure to hold allof the platform-specific time values */
struct time_generic {
#ifndef _WIN32
	time_t tv_sec;
	long tv_nsec;
...
void usage(void) {
	fprintf(stderr, "fl2k_test, clock accuracy test for FL2K VGA dongles, also outputs a square wave at fs/2\n\n"
			"Usage:\n"
...
static int ppm_report(uint64_t nsamples, uint64_t interval) {
	...
	ppm = 1e6 * (real_rate / (double)samp_rate - 1.0);
...
static void ppm_test(uint32_t len) {
	...
	if (ppm_init != PPM_INIT_RUN) {
		 ...
		 * The dongle's PPM is usually very bad when first starting up, typically incorrect by more than twice the final
		 * value. Discarding the first few seconds allows the value to stabilize much faster.
		 */
...
void fl2k_callback(fl2k_data_info_t *data_info) {
	...
	if (cb_cnt > 20) {
		...
		/* as data is repetitive, it only needs to be handed over until all transfer buffers contain the data */
