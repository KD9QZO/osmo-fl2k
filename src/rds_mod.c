...
/*! \brief Structure to hold RDS parameters */
struct {
	uint16_t pi;				/*!< The \b PI (program information) data */
	int ta;
	char ps[PS_LENGTH + 1];
	char rt[RT_LENGTH + 1];		/*!< The \b RT (radio text) data */
} rds_params = { 0 };


/**
 * \name RDS Error-Detection Code Generator Polynomial
 *
 * The RDS error-detection code generator polynomial is:
 * \f[ x^10 + x^8 + x^7 + x^5 + x^4 + x^3 + x^0 \f]
 *
 * @{
*/
...
/**
 * \brief Possibly generates a CT group
 *
 * This function possibly generates a \b CT (clock time) group, if the minute has just changed.
 *
 * \param[out]	blocks	Pointer to the array of RDS data blocks; the \b CT group data will be written here
 *
 * \return Indicates whether the \b CT group was generated.
 * \retval	0	The \b CT group was \e NOT generated
 * \retval	1	The \b CT group was generated
 */
int get_rds_ct_group(uint16_t *blocks) {
	...
	if (utc->tm_min != latest_minutes) {
		...
		l = utc->tm_mon <= 1 ? 1 : 0;
		mjd = 14956 + utc->tm_mday + (int)((utc->tm_year - l) * 365.25) + (int)((utc->tm_mon + 2 + l * 12) * 30.6001);

		blocks[1] = (0x4400 | (mjd >> 15));
		blocks[2] = ((mjd << 1) | (utc->tm_hour >> 4));
		blocks[3] = (((utc->tm_hour & 0x0F) << 12) | (utc->tm_min << 6));
...
/**
 * \brief Get a number of RDS samples
 *
 * This generates the envelope of the waveform using pre-generated elementary waveform samples, and then it
 * amplitude-modulates the envelope with a 57 kHz carrier; this is very efficient, as 57 kHz is 4 times the sample
 * frequency we are working at (228 kHz).
 */
void get_rds_samples(double *buffer, uint32_t count) {
	...
	for (i = 0; i < count; i++) {
		...
		// modulate at 57 kHz; use phase for this
		switch (phase) {
			case 0:
			case 2:
				sample = 0;
